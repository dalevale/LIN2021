...
void add_string(char *str)
{
    ...
    new_node->data = vmalloc(sizeof(char)*strlen(str));
...
{
    char command[20];
    char *str=vmalloc(sizeof(char)*PROCFS_MAX_SIZE);
    int nr_bytes_num=0;
    int nr_bytes_buf=0;
    ...
    if(sscanf(command, "add %s", str)==1){
        /* Añadir nodo */
        /* Comprobar tamaño del buffer */
		p+=sprintf(p, "%s\n", str);
		nr_bytes_num=p-kbuf;
		printk(KERN_INFO "Modlist: Size of string %d\n", nr_bytes_num);
		p=kbuf;
	    ...
	    printk(KERN_INFO "Modlist: Añadiendo el string %s\n", str);
	    add_string(str);

    } else if (sscanf(command, "remove %s", str)==1){
		/* Eliminar nodo */
		printk(KERN_INFO "Modlist: Eliminando el string %s\n", str);
		if(list_empty(&mylist)){
