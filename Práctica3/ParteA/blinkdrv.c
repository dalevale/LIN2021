			  ...
			  size_t len, loff_t *off)
{
	...
	if(kbuf[0]=='\n'){
		...
		while((str = strsep(&kbuf, ",")) != NULL){
	   		if((strlen(str)==0) || (str[0]=='\n'))
	   			printk(KERN_ALERT "Entrada invalida");
	   		else {
	   			args = sscanf(str, "%d:%x", &led, &color);
	   			if((led < NR_LEDS) && (led >= 0) && (args >= 1)){
	   				r = (color & 0xff0000) >> 16;
					g = (color & 0xff00) >> 8;
					b = color & 0xff;
					message[0]='\x05';
					...
					if (retval<0){
	   				...
	   				printk(KERN_ALERT "Invalid led number: %d", led);
